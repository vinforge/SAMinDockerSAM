...
def extract_content_from_memory_result(memory_result: Any) -> str:
    ...
    try:
        ...
        logger.warning(f"Error extracting content from memory result: {e}")
        return str(memory_result)
...
def extract_metadata_from_memory_result(memory_result: Any) -> Dict[str, Any]:
    ...
    try:
        # Phase 3: RankedMemoryResult
        if hasattr(memory_result, 'metadata') and hasattr(memory_result, 'final_score'):
